/**
 * Cross-platform fixture parity tests for `@blockchaincommons/sskr`.
 *
 ...
 * with the TypeScript `combineShares`. This proves bidirectional interop:
 *
 ...
 *     through TS's `generateShares` path — i.e. given a third party
 ...
 * below diverge from `combineShares`'s expectation and these tests fail
...
describe("Cross-platform Rust → TS SSKR share decoding", () => {
  ...
  it("rejects a Rust-produced share with corrupted reserved nibble", () => {
    ...
    try {
      ...
      expect((e as SskrError).code).toBe("ShareReservedBitsInvalid");
    }
  });
  ...
  it("rejects a Rust-produced share that is too short", () => {
    ...
    try {
      ...
      expect(e).toBeInstanceOf(SskrError);
      expect((e as SskrError).code).toBe("ShareLengthInvalid");
    }
  });
