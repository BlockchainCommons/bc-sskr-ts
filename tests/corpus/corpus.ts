/**
 * Deterministic differential corpus (Phase 1.3). Pure and deterministic.
 */
import type { Recipe, Bytes, RngSpec, SpecShape, GenSpec } from "../vectors/recipes";
...
const spec = (gt: number, ...groups: { mt: number; mc: number }[]): SpecShape => ({ gt, groups });

/** Every (t, n) with 1 ≤ t ≤ n ≤ 5. */
const SMALL: { mt: number; mc: number }[] = [];
...
const positions = (s: SpecShape): [number, number][] =>
  s.groups.flatMap((grp, gi) =>
    Array.from({ length: grp.mc }, (_, mi) => [gi, mi] as [number, number]),
  );
function* combine(): Generator<Recipe> {
  ...
  for (const from of bases) {
    ...
    const pick = pos.slice(0, Math.max(2, from.spec.gt * 2));
